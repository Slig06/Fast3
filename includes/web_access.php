...
class Webaccess{
	...
	function request($url, $callback, $datas, $is_xmlrpc=false, $keepalive_min_timeout=300, $opentimeout=3, $waittimeout=5,$agent='XMLaccess'){
		...
		if($host===false)
			console('*Webaccess request(): Bad url: '.$url);
	...
	function retry($url){
		list($host,$port,$path) = getHostPortPath($url);
		if($host===false)
			console('*Webaccess retry(): Bad url: '.$url);
		else{
			$server = $host.':'.$port;
...
class WebaccessUrl{
  ...
  //-----------------------------
	...
	function _bad($errstr,$isbad=true){
		...
		console('*'.$this->_webaccess_str.$errstr);
	...
	//               'IsXmlrpc'=>$is_xmlrpc, 'KeepaliveMinTimeout'=>$keepalive_min_timeout,
	//               'OpenTimeout'=>$opentimeout, 'WaitTimeout'=>$waittimeout);
	// will add:     'State','HDatas','Datas','DatasSize','DatasSent','Response','ResponseSize','Headers','Close','Times'
  function request(&$query){
		global $_web_access_compress_reply,$_web_access_post_xmlrpc,$_web_access_retry_timeout,$_web_access_retry_timeout_max;
		//$post_xmlrpc=true;
		$query['State'] = 'BAD';
		$query['HDatas'] = '';
		$query['Datas'] = '';
		...
		$query['Headers'] = array();
		$query['Close'] = false;
		$query['Times'] = array('open'=>array(-1.0,-1.0),'send'=>array(-1.0,-1.0),'receive'=>array(-1.00,-1.0,0));
		...
		if(($query['Callback']!=null) && ($this->_state=='BAD')){
			if($this->_bad_timeout > $_web_access_retry_timeout_max){
				console('*'.$this->_webaccess_str.'Request refused for consecutive errors ('.$this->_bad_timeout." / ".$_web_access_retry_timeout_max.")");
				return false;
		...
		if(($query['Callback']==null) || (is_array($query['Callback']) && 
			...
			if(is_string($query['QueryDatas']) && strlen($query['QueryDatas'])>0){
				...
				console('*'.$this->_webaccess_str.'Bad datas');
				return false;
			}
  ...
  // open the socket (close it before if needed)
	private function _open_socket($opentimeout=0.0){
		...
		if(!$this->_socket || $this->_state != 'OPENED'){
			...
			$this->_state = 'OPENED';
			//console('*'.$this->_webaccess_str."connection opened !");
  ...
  // open the connection (if not already opened) and send
	function _open($opentimeout=0.0, $waittimeout=5.0){
		...
		if(!$this->_wait && $this->_state=='BAD' &&
			...
			//console('*'.$this->_webaccess_str.' wait to retry ('.($time-$this->_bad_time)." / ".$this->_bad_timeout.")");
			return false;
		}
		...
		if($this->_socket && $this->_state=='OPENED' &&
			...
			//console('*'.$this->_webaccess_str."timeout, close it !");
			$this->_state = 'CLOSED';
			@fclose($this->_socket);
			$this->_socket = null;
		}
	...
	function _send($waittimeout=20){
		...
		if($this->_spool[0]['State']=='OPEN'){
			...
			if(count($this->_cookies)>0){
				...
				foreach($this->_cookies as $name => $cookie){
					if(!isset($cookie['path']) || strncmp($this->_spool[0]['Path'],$cookie['path'],strlen($cookie['path']))==0){
		...
		do{
			$sent = @stream_socket_sendto($this->_socket,substr($this->_spool[0]['Datas'],
			...
			if($sent==false){
				...
				if($this->_spool[0]['DatasSent'] >= $this->_spool[0]['DatasSize']){
					...
					// if timeout then error
					$this->_bad("Request timeout, in _send ({$difftime} > {$waittimeout}s) state=".$this->_spool[0]['State']);
					break;
				}
	...
	function _receive($waittimeout=40){
		...
		if(!isset($this->_spool[0]['State']) || $this->_spool[0]['State'] != 'RECEIVE'){
			...
			if($this->_socket && @feof($this->_socket)){
				//console('Socket closed by server ('.$this->_host.')');
				$this->_state = 'CLOSED';
				@fclose($this->_socket);
				$this->_socket = null;
			}
		...
		if($state===true){
			...
			if(!$this->_keepalive || $this->_spool[0]['Close']){
				...
				//console('*'.$this->_webaccess_str."close connection (asked in headers)");
				$this->_state = 'CLOSED';
				@fclose($this->_socket);
				$this->_socket = null;
			}
  ...
  private function _handleHeaders(){
    ...
    if(count($datas)<2){
      ...
      if(count($datas)<2){
		...
		if(!isset($this->_spool[0]['Headers']['Command'][0])){
			// separate headers
			//echo "Get Headers !(".strlen($datas[1]).")\n";
		...
		if($this->_compress_request=='accept' && isset($headers['accept-encoding'][0])){
			...
			console($this->_webaccess_str.'send: '.($this->_compress_request===false?'no compression':$this->_compress_request)
		...
		if(!isset($headers['connection'][0]) || $headers['connection'][0]=='close'){
			//if(!$this->_spool[0]['Close'])
			//console('*'.$this->_webaccess_str."server ask to close connection.");
			$this->_spool[0]['Close'] = true;
		}
		...
		if(isset($headers['keep-alive'])){
			...
			//console('*'.$this->_webaccess_str."max=".$this->_serv_keepalive_max.", timeout=".$this->_serv_keepalive_timeout.".\n");
		}
	...
	function infos(){
		...
		$msg = $this->_webaccess_str
			.sprintf("[%s,%s]: %0.3f / %0.3f / %0.3f (%0.3f) / %d [%d,%d]",
							 $this->_state,$this->_spool[0]['State'],
							 $this->_spool[0]['Times']['open'][1], $this->_spool[0]['Times']['send'][1],
							 $this->_spool[0]['Times']['receive'][1], $this->_spool[0]['Times']['receive'][2],
							 $this->_query_num,$size,$this->_spool[0]['ResponseSize']);
		console($msg);
	}
}
